...
class bmp280(object):
    ...
    def __init__(self, i2c_bus=None, addr=ADDR):
        ...
        if (
            getattr(i2c_bus, "write_byte_data", None) is None
            or getattr(i2c_bus, "read_byte_data", None) is None
        ):
            raise TypeError(
                "Object given for i2c_bus must implement write_byte_data and read_byte_data methods"
            )
    ...
    def _read_word(self, register):
        # read_word_data isn't implemented, so we make a word ourselves
        # return self.i2c_bus.read_word_data(self.addr, register)

        byte1 = self.i2c_bus.read_byte_data(self.addr, register)
        byte2 = self.i2c_bus.read_byte_data(self.addr, register)
